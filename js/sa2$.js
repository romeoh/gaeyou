...
function ready() {
	...
	$.ajax({
		...
		,'success': function(result){
			...
			M('#profile').attr('src', 'http://romeoh.github.io/gaeyou/images/imoticon/' + profile + '.png');
			M('#uname').html(uname);
			var panel = ''
			if (deleteable || admin) {
				panel += M.dynamicDate(regDate) + ' <span class="del" data-del="' + idx + '"> | 삭제</span>'
			} else {
				panel += M.dynamicDate(regDate);
			}
			if (admin) {
				panel += '<span class="del" data-server="' + idx + '"> | ' + server + '</span>'
			}
			M('#ddate').html(panel);
			...
			// 사진, 동영상 표시
			if (photo != '') {
				M('#imgCon').html('<img src="' + server + 'upload/sa/large/' + photo + '">');
				...
				.on('click', function(evt, mp){
					...
					$.ajax({
						...
						,'success': function(result){
							...
							post += '────────────────────\n'
							post += content + '\n\n';
							post += 'http://gaeyou.com/s2/#' + cuData['idx'];
							
							if (type == 'movie') {
								cont = 'http://i1.ytimg.com/vi/' + cuData['movie'] + '/default.jpg';
							} else {
								cont = 'http://www.gaeyou.com/upload/sa/large/' + cuData['photo'];
							}
							//console.log(cont)
							urlinfo = {
								'title': '😜 움직이는 짤방',//'깨알움짤',
					...
					$.ajax({
						...
						,'success': function(result){
							...
							kakao.link('talk').send({
								...
								url: 'http://gaeyou.com/s2/#' + cuData['idx'],
								appid: 'gaeyou',
								appver: '1.0',
								appname: '깨알웃사',
								type: 'link'
			...
			// 서버변경
			M('[data-server]').on('click', function(evt, mp){
				if (cuData.server == 'http://gaeyou.com/') {
					s = 'http://romeoh.github.io/gaeyou/';
				} else {
					s = 'http://gaeyou.com/';
				}
				bodyData = {
				...
				$.ajax({
					 'url': apiurl + 'sa_server.php'
					,'contentType': 'application/x-www-form-urlencoded'
					,'data': bodyData
					,'type': 'POST'
					,'success': function(result){
						M('[data-server]').html(s)
					}
				})
			})
			getReplys();
	...
	$.ajax({
		...
		,'success': function(result) {
			...
			for (var i=0; i<result.length; i++) {
				...
				str += '	<a href="/s2/#' + idx + '" style="display:inline">' + contents + '</a>';
				str += '	<div class="info2">';
		...
		,'success': function(result){
			...
			if (result.length == 0) {
				...
				for (var i=0; i<result.length; i++) {
					...
					str += '<div class="rlist">';
					str += '	<div class="imo"><img src="http://romeoh.github.io/gaeyou/images/imoticon/' + photo + '.png"></div>';
...
function getIndexList() {
	...
	$.ajax({
		 'url': apiurl + code + '_get_index_all.php'
		,'contentType': 'application/x-www-form-urlencoded'
		,'data': bodyData
		,'type': 'POST'
		,'success': function(result) {
			...
			prev = cuIdx - 1;
			next = cuIdx + 1;
			M('#prev').attr('href', 'http://gaeyou.com/s2/#' + result[prev]);
			M('#next').attr('href', 'http://gaeyou.com/s2/#' + result[next]);
			
			if (prev == '-1') {
				M('#prev').css('display', 'none');
			}
